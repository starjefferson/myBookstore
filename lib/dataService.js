...
import { INITIAL_BOOKS, INITIAL_ORDERS } from "./mockData";
...
export const fetchOrders = async (userEmail = null) => {
  if (isLiveFirebaseConfigured && db) {
    try {
      let q;
      if (userEmail) {
        q = query(collection(db, "orders"), where("buyerEmail", "==", userEmail), orderBy("createdAt", "desc"));
      } else {
  ...
  const orders = getLocalData(LOCAL_STORAGE_ORDERS_KEY, INITIAL_ORDERS);
  if (userEmail) {
